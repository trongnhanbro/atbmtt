...
def browse_file_mahoa():
    file_path = filedialog.askopenfilename()
    with open(file_path, 'r') as file:
        file_contents = file.read()
    inputVanbangoc.delete(0, END)
    inputVanbangoc.insert(INSERT, file_contents)
    # print(file_contents)
    # inputVanbangoc.delete()


btnMahoa = Button(window, text="Mã Hóa", command=mahoa_DES)
btnMahoa.grid(column=0, row=7)

btnFileMahoa = Button(window, text="File Van Ban Goc",
                      command=browse_file_mahoa)
btnFileMahoa.grid(column=0, row=8)
